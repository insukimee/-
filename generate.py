...
add("spcx", "SPCX", "space", "비상장 (Private)", "스페이스X (비상장)", "スペースX（非上場）", "2026.06.10",
    "민간 우주 산업의 압도적 1위 기업 스페이스X를 분석합니다. 비상장 기업으로 일반 주식 시장에서 직접 거래되지 않습니다.",
    "民間宇宙産業の圧倒的首位企業であるスペースXを分析します。非上場企業であり、一般株式市場で直接取引されません。",
    [
     ("스페이스X는 재사용 로켓(팰컨9), 위성 인터넷(스타링크), 차세대 스타십을 보유한 비상장 우주 기업입니다. 발사 시장을 사실상 장악했으며, 스타링크 가입자 확대로 매출이 빠르게 성장하고 있습니다.",
      "スペースXは再使用ロケット（ファルコン9）、衛星インターネット（スターリンク）、次世代スターシップを擁する非上場の宇宙企業です。打ち上げ市場を事実上掌握し、スターリンク加入者の拡大で売上が急成長しています。"),
     ("스타링크의 가입자·매출 급증, 발사 시장 독점적 지위, 스타십 상용화 시 비용 혁신 잠재력, 정부·국방 계약이 핵심 가치 동인입니다.",
      "スターリンクの加入者・売上の急増、打ち上げ市場での独占的地位、スターシップ商用化時のコスト革新ポテンシャル、政府・防衛契約が中核的な価値ドライバーです。"),
     ("비상장으로 일반 투자자의 직접 투자가 어렵고 유동성이 낮으며, 막대한 자본 소요, 스타십 개발·인증 리스크, 기업가치 평가의 불확실성이 리스크입니다.",
      "非上場のため一般投資家の直接投資が難しく流動性が低いこと、巨額の資本需要、スターシップ開発・認証リスク、企業価値評価の不確実性がリスクです。"),
     ("우주 경제의 핵심 기업이나 비상장이므로, 직접 매수보다는 IPO 가능성·간접 노출(관련 펀드 등)을 모니터링하는 관점이 현실적입니다.",
      "宇宙経済の中核企業ですが非上場のため、直接購入よりもIPOの可能性・間接的なエクスポージャー（関連ファンド等）をモニタリングする視点が現実的です。"),
    ])
...
def render_about(lang):
    ...
    if lang == "ko":
      ...
      <h2>면책 조항</h2>
      <p>본 사이트의 모든 정보는 일반적인 투자 참고 목적으로만 제공되며, 특정 금융 상품의 매수·매도를 권유하는 투자 권유가 아닙니다. 또한 실시간 시세가 아닌 일반적·정성적 분석이며, 투자 결정의 최종 책임은 투자자 본인에게 있습니다.</p>"""
    else:
      ...
      <h2>免責事項</h2>
      <p>本サイトのすべての情報は一般的な投資参考目的のみで提供されており、特定の金融商品の売買を勧誘する投資勧誘ではありません。また、リアルタイムの株価ではなく一般的・定性的な分析であり、投資決定の最終的な責任は投資家ご自身にあります。</p>"""
...
def render_terms(lang):
    ...
    if lang == "ko":
      ...
      <h2>제3조 (면책 조항)</h2>
      <ul>
        <li>사이트의 정보는 정확성·완전성을 보증하지 않으며, 실시간 시세가 아닙니다.</li>
      ...
      <h2>第3条（免責事項）</h2>
      <ul>
        <li>当サイトの情報は正確性・完全性を保証せず、リアルタイムの株価でもありません。</li>
